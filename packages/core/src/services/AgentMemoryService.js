...
export class AgentMemoryService {
    constructor(options, memoryManager) {
     ...
     * Add a memory
     */
    async add(content, type, namespace, metadata = {}) {
        const memory = {
            id: this.generateId(),
            type,
            namespace,
    ...
    async search(query, options = {}) {
        ...
        const mappedResults = vectorResults.map(r => ({
            id: r.id,
            content: r.content,
            type: r.metadata?.type || 'long_term',
            namespace: r.metadata?.namespace || 'project',
            metadata: r.metadata,
            createdAt: new Date(r.metadata?.createdAt || Date.now()),
            accessedAt: new Date(),
        ...
        if (options.tags?.length) {
            results = results.filter(m => options.tags.some(tag => m.metadata.tags?.includes(tag)));
        }
     ...
     */
    getStats() {
        const byType = { session: 0, working: 0, long_term: 0 };
        const byNamespace = { user: 0, agent: 0, project: 0 };
