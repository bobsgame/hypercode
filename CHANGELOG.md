...
- **Claude-Mem Workspace-Name-Aware Build Exclusion**: `scripts/build_all.mjs` now reads the real workspace package name for `packages/claude-mem` and excludes both the logical `claude-mem` label and the actual Turbo package identity when the merge-marker safety path triggers, preventing that escape hatch from silently missing the real workspace package.
...
- **Go-Backed MCP Dashboard Mutation Compatibility**: the shared Next.js `/api/trpc/[trpc]` compatibility route now translates key MCP inspector/search/system mutations onto the existing Go `/api/mcp/*` surface when `/trpc` is unavailable, including `mcp.setToolPreferences`, `mcp.loadTool`, `mcp.unloadTool`, `mcp.clearToolSelectionTelemetry`, `mcp.clearWorkingSetEvictionHistory`, and `mcp.setLifecycleModes`.
- **Go-Backed Session Dashboard Compatibility**: the shared Next.js `/api/trpc/[trpc]` compatibility route now translates the session dashboard's key session-supervisor reads and mutations onto the existing Go `/api/sessions/supervisor/*` surface when `/trpc` is unavailable, including `session.get`, `session.logs`, `session.attachInfo`, `session.health`, `session.create`, `session.start`, `session.stop`, `session.restart`, `session.executeShell`, `session.updateState`, and `session.clear`.
...
- **Execution-Policy Parity for Go Fallback Sessions**: Go fallback supervised sessions now persist and expose `executionPolicy` metadata plus execution-policy env markers, and native `session.executeShell` now follows the session's chosen policy/shell instead of only using a generic fallback shell.
- **Durable Go Supervised-Session Lifecycle Fallback**: the public `/api/sessions/supervisor/{list,get,create,start,stop,restart,logs,attach-info,health,execute-shell,restore}` routes now preserve truthful operator behavior when the TypeScript runtime is unavailable by falling back to a native Go supervised-session manager with durable Go-config-dir persistence (`.hypercode-go/session-supervisor.json`), explicit restore-on-demand support, restart-aware restore normalization, log buffering, attach readiness, health tracking, and one-shot shell execution in the session working directory.
...
- **HyperCode Harness Assimilation**: Added `submodules/hyperharness` as a tracked upstream and introduced a shared HyperCode CLI harness registry so `hypercode` is now the primary `hypercode session` harness identity.
- **HyperCode Tool Inventory Visibility**: HyperCode CLI and the Go sidecar harness registry now surface HyperCode's source-backed tool calls by reading `submodules/hyperharness/tools/*.go`, while keeping other external harnesses labeled as install/runtime metadata only until deeper bridge contracts exist.
...
### Fixed
- **Go-Primary Startup Phase Truthfulness**: `start.bat` now captures Go-primary install/build probe exit codes with runtime `!ERRORLEVEL!` instead of parse-time `%ERRORLEVEL%`, fixing contradictory startup output where the probe could report `build required` and the script would still claim the build was skipped. Go-primary installs now also default to `pnpm install --ignore-scripts` unless `HYPERCODE_STARTUP_INSTALL_SCRIPTS=1` is set, reducing unnecessary failures from unrelated workspace postinstall hooks during Go-only startup.
- **Go Execution-Environment Compat Truth**: the web tRPC compatibility layer now prefers Go-native `/api/tools/detect-execution-environment` when `tools.detectExecutionEnvironment` is unavailable, and degraded `startupStatus.checks.executionEnvironment` now reuses that same normalized native summary instead of falling back to all-zero synthetic placeholders.
- **Go Install-Surface Compat Truth**: the web tRPC compatibility layer now prefers Go-native `/api/tools/detect-install-surfaces` when `tools.detectInstallSurfaces` is unavailable, preserving browser-extension, VS Code, and MCP sync artifact summaries in degraded mode instead of returning empty install-surface inventories.
- **Go Imported-Session Maintenance Compat Truth**: the web tRPC compatibility layer now prefers Go-native `/api/sessions/imported/maintenance-stats` when `session.importedMaintenanceStats` is unavailable, and degraded `startupStatus.checks.importedSessions` now backfills from that same native maintenance surface when startup telemetry omits imported-session archive counters.
- **Go MCP Inspector Compat Truth**: the web tRPC compatibility layer now prefers Go-native `/api/mcp/working-set`, `/api/mcp/tool-selection-telemetry`, and `/api/mcp/preferences` when `mcp.getWorkingSet`, `mcp.getToolSelectionTelemetry`, and `mcp.getToolPreferences` are unavailable, preserving inspector working-set state, telemetry history, and preference controls instead of synthetic empty placeholders.
- **Go Operator Read Compat Truth**: the web tRPC compatibility layer now also prefers Go-native `/api/api-keys`, `/api/shell/history/system`, `/api/memory/agent-stats`, and `/api/expert/status` when `apiKeys.list`, `shell.getSystemHistory`, `agentMemory.stats`, and `expert.getStatus` are unavailable, preserving operator-facing metadata and status instead of synthetic empty placeholders.
- **Go Tool Catalog/Search Compat Truth**: the web tRPC compatibility layer now also prefers Go-native `/api/tools` and `/api/tools/search` when `tools.list` and `mcp.searchTools` are unavailable, preserving tool inventory and search results instead of synthetic empty catalog/search placeholders.
- **Go MCP Traffic/Health Compat Truth**: the web tRPC compatibility layer now also prefers Go-native `/api/mcp/traffic` and `/api/server-health/check` when `mcp.traffic` and UUID-backed `serverHealth.check` are unavailable, preserving router traffic rows and truthful server health counters instead of synthetic empty traffic and config-only health inference.
- **Go Session-State Compat Truth**: the web tRPC compatibility layer now also prefers Go-native `/api/sessions/supervisor/state` when `session.getState` is unavailable, preserving truthful session-state signals like active auto-drive and current goal instead of a synthetic placeholder.
