...
| Session state/log parity | Bridge-first with Partial Native Go fallback | Go now has native persisted fallback ownership for the shared session-state core (`getState`, `updateState`, `clear`, `heartbeat`) via workspace `.hypercode-session.json`, plus native persisted supervisor lifecycle/log/attach/health/session-shell fallback behavior for public supervised sessions; execution-policy visibility is now much closer to TS parity, but fuller worktree/isolation and memory-bootstrap parity are still incomplete |
