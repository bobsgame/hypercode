...
export const FileSystemTools = [
    ...
    },
    {
        ...
        handler: async (args: { file_path: string, start_line?: number, end_line?: number }) => {
            try {
    ...
    },
    {
        ...
        handler: async (args: { file_path: string, content: string }) => {
            try {
                await fs.mkdir(path.dirname(args.file_path), { recursive: true });
                await fs.writeFile(args.file_path, args.content, "utf-8");
                return { content: [{ type: "text", text: `Successfully wrote to ${args.file_path}` }] };
            } catch (err: any) {
                return { content: [{ type: "text", text: `Error: ${err.message}` }] };
            }
        }
    },
    {
        ...
        handler: async (args: { file_path: string, old_string: string, new_string: string }) => {
            try {
                const content = await fs.readFile(args.file_path, "utf-8");
                if (!content.includes(args.old_string)) {
                    return { content: [{ type: "text", text: `Error: old_string not found in file. Ensure exact match including whitespace.` }] };
                }
                const newContent = content.replace(args.old_string, args.new_string);
                await fs.writeFile(args.file_path, newContent, "utf-8");
                return { content: [{ type: "text", text: `Successfully replaced text in ${args.file_path}` }] };
            } catch (err: any) {
                return { content: [{ type: "text", text: `Error: ${err.message}` }] };
            }
        }
    ...
    // Aliases for Borg-native compatibility
    {
        ...
        handler: async (args: any) => {
            const replacer = FileSystemTools.find(t => t.name === "str_replace_editor");
            return replacer!.handler({ file_path: args.path, old_string: args.oldText, new_string: args.newText });
        }
    }
];
