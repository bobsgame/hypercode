...
| Surface | Current status | Notes |
|---|---|---|
| MCP inventory snapshot | Bridge-first / Partial Native Go | Go inventory now reads live `mcp.jsonc`, local `metamcp.db`, and a Go-owned persisted inventory cache file (`mcp_inventory_cache.json`) in the Go config dir; JSONC save/mutation flows now actively resync that cache from live sources, and fallback inventory views can overlay runtime-registry live-probed tools while surfacing cache freshness/source metadata |
...
| Metadata refresh/cache management | Partial Native Go | native JSONC inspection/cache normalization exists, probeable STDIO servers can attempt live `tools/list` refresh, and local JSONC write paths now resync the Go inventory cache from live sources so metadata clear/refresh actions do not leave stale inventory cache state behind; broader multi-transport live discovery parity is still incomplete |
